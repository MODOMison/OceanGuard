...
class OceanGuardApp:
    def __init__(self, root):
        ...
        try:
            ...
            self.bg_image = self.bg_image.resize((600, 500), Image.Resampling.LANCZOS)  # Resize to fit the window
        ...
        # Welcome Screen
        self.welcome_label = tk.Label(root, text="Welcome to OceanGuard", font=("Arial", 16), bg="lightblue")
        self.welcome_label_window = self.canvas.create_window(300, 50, window=self.welcome_label)

        self.name_entry = tk.Entry(root, width=30, font=("Arial", 12))
        self.name_entry_window = self.canvas.create_window(300, 100, window=self.name_entry)
        self.name_entry.insert(0, "Enter your name")

        self.start_button = tk.Button(root, text="Start", command=self.start_app, font=("Arial", 12), bg="lightgreen")
        self.start_button_window = self.canvas.create_window(300, 150, window=self.start_button)
    ...
    def create_main_menu(self):
        ...
        for text, command in buttons:
            button = tk.Button(self.root, text=text, command=command, width=20, font=("Arial", 12), bg="lightgreen")
            self.canvas.create_window(300, y_offset, window=button)
            y_offset += 40
